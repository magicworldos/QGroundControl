...
public:
	...
	inline BlockType &operator=(const SparseMatrixBase<OtherDerived> &other)
	{
		...
		Index start         = m_outerStart == 0 ? 0 :
				      matrix.outerIndexPtr()[m_outerStart]; // starting position of the current block
		Index end           = m_matrix.outerIndexPtr()[m_outerStart +
...
public:
	...
	public:
		...
		{
			while ((Base::operator bool())
					&& (Base::index() >= (IsRowMajor ? m_block.m_startCol.value() + block.m_blockCols.value() : m_block.m_startRow.value() +
							      block.m_blockRows.value())))
			{
				Base::operator--();
			}
