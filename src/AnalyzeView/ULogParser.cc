		...
		case (int)ULogMessageType::DATA:
			{
				...
				if (msgID == _cameraCaptureMsgID)
				{
					...
					memcpy(&feedback.timestampUTC, log.data() + index + 5 + _cameraCaptureOffsets.value(QStringLiteral("timestamp_utc")),
					       8);
					...
					memcpy(&feedback.groundDistance, log.data() + index + 5 + _cameraCaptureOffsets.value(
						       QStringLiteral("ground_distance")), 4);
