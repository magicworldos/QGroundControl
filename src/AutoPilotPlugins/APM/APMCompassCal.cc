...
CalWorkerThread::calibrate_return CalWorkerThread::calibrate(void)
{
	...
	if (result == calibrate_return_ok)
	{
		for (unsigned cur_mag = 0; cur_mag < max_mags; cur_mag++)
		{
			if (rgCompassAvailable[cur_mag])
			{
				_emitVehicleTextMessage(QStringLiteral("[cal] mag #%1 off: x:%2 y:%3 z:%4").arg(cur_mag).arg(-sphere_x[cur_mag]).arg(
								-sphere_y[cur_mag]).arg(-sphere_z[cur_mag]));
...
CalWorkerThread::calibrate_return CalWorkerThread::mag_calibration_worker(detect_orientation_return orientation,
		void *data)
{
	calibrate_return result = calibrate_return_ok;
	...
	_emitVehicleTextMessage(QStringLiteral("[cal] Continue rotation for %1 seconds").arg(
	...
	unsigned int loop_interval_usecs = (worker_data->calibration_interval_perside_seconds * 1000000) /
			...
			&& calibration_counter_side < worker_data->calibration_points_perside)
	{
		...
		else
		{
			...
			_emitVehicleTextMessage(QStringLiteral("[cal] %1 side calibration: progress <%2>").arg(detect_orientation_str(
							orientation)).arg(progress_percentage(worker_data) +
	...
	if (result == calibrate_return_ok)
	{
		_emitVehicleTextMessage(QStringLiteral("[cal] %1 side done, rotate to a different side").arg(detect_orientation_str(
						orientation)));
...
void APMCompassCal::startCalibration(void)
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		{
			_calWorkerThread->rgCompassAvailable[i] = _vehicle->parameterManager()->getParameter(-1,
					deviceIdParam)->rawValue().toInt() > 0;
		...
		qCDebug(APMCompassCalLog) << QStringLiteral("Compass %1 available: %2").arg(i).arg(
