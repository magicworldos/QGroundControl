...
void APMSensorsComponentController::_mavCommandResult(int vehicleId, int component, int command, int result,
		bool noReponseFromVehicle)
{
	...
	if (command == MAV_CMD_DO_CANCEL_MAG_CAL)
	{
		...
		if (result == MAV_RESULT_ACCEPTED)
		{
			...
			_appendStatusLog(
				tr("Rotate the vehicle randomly around all axes until the progress bar fills all the way to the right ."));
...
void APMSensorsComponentController::_handleUASTextMessage(int uasId, int compId, int severity, QString text)
{
	...
	if (text.endsWith(QLatin1Literal("side done, rotate to a different side")))
	{
		...
		_orientationCalAreaHelpText->setProperty("text",
				tr("Place you vehicle into one of the orientations shown below and hold it still"));
...
void APMSensorsComponentController::_handleMagCalProgress(mavlink_message_t &message)
{
	if (_calTypeInProgress == CalTypeOnboardCompass)
	{
		...
		if (_progressBar)
		{
			_progressBar->setProperty("value", (float)(_rgCompassCalProgress[0] + _rgCompassCalProgress[1] +
						  _rgCompassCalProgress[2]) / 100.0);
		}
	}
}
...
void APMSensorsComponentController::_handleMagCalReport(mavlink_message_t &message)
{
	if (_calTypeInProgress == CalTypeOnboardCompass)
	{
		...
		if (_rgCompassCalComplete[0] && _rgCompassCalComplete[1] && _rgCompassCalComplete[2])
		{
			for (int i = 0; i < 3; i++)
			{
				qCDebug(APMSensorsComponentControllerLog) << QString("Onboard compass call report #%1: succeed:fitness %2:%3").arg(
							i).arg(_rgCompassCalSucceeded[i]).arg(_rgCompassCalFitness[i]);
			}
