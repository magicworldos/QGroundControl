...
ESP8266ComponentController::wifiIPAddress()
{
	if (_ipAddress.isEmpty())
	{
		...
		{
			QHostAddress address(qFromBigEndian(getParameterFact(MAV_COMP_ID_UDP_BRIDGE,
							    QStringLiteral("WIFI_IPADDRESS"))->rawValue().toUInt()));
...
//-----------------------------------------------------------------------------
void
ESP8266ComponentController::_mavCommandResult(int vehicleId, int component, int command, int result,
