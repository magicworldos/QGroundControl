...
const int RadioComponentController::_updateInterval =
	150;              ///< Interval for timer which updates radio channel widgets
const int RadioComponentController::_rcCalPWMCenterPoint = ((RadioComponentController::_rcCalPWMValidMaxValue -
		RadioComponentController::_rcCalPWMValidMinValue) / 2.0f) + RadioComponentController::_rcCalPWMValidMinValue;
...
const int RadioComponentController::_rcCalSettleDelta =
	20;     ///< Amount of delta which is considered no stick movement
...
const RadioComponentController::stateMachineEntry *RadioComponentController::_getStateMachineEntry(int step) const
{
	...
	static const char *msgBeginAPM =
		QT_TR_NOOP("Lower the Throttle stick all the way down as shown in diagram.\nReset all transmitter trims to center.\n\n"
...
void RadioComponentController::nextButtonClicked(void)
{
	if (_currentStep == -1)
	{
		...
		if (_chanCount < _chanMinimum)
		{
			...
			{
				qgcApp()->showMessage(QString("Detected %1 radio channels. To operate PX4, you need at least %2 channels.").arg(
							      _chanCount).arg(_chanMinimum));
			}
...
bool RadioComponentController::_stickSettleComplete(int value)
{
	...
	if (abs(_stickDetectValue - value) > _rcCalSettleDelta)
	{
		...
		qCDebug(RadioComponentControllerLog) << "_stickSettleComplete still moving, _stickDetectValue:value" <<
	...
	else
	{
		...
		else
		{
			...
			qCDebug(RadioComponentControllerLog) << "_stickSettleComplete starting settle timer, _stickDetectValue:value" <<
							     _stickDetectValue << value;
...
{
	qCDebug(RadioComponentControllerLog) << "_inputStickDetect function:channel:value" <<
					     _functionInfo()[function].parameterName << channel << value;
	...
	if (_stickDetectChannel == _chanMax())
	{
		...
		if (abs(_rcValueSave[channel] - value) > _rcCalMoveDelta)
		{
			...
			qCDebug(RadioComponentControllerLog) << "_inputStickDetect starting settle wait, function:channel:value" << function <<
							     channel << value;
	...
	else if (channel == _stickDetectChannel)
	{
		if (_stickSettleComplete(value))
		{
			...
			qCDebug(RadioComponentControllerLog) << "_inputStickDetect settle complete, function:channel:value:reversed" << function
							     << channel << value << info->reversed;
...
void RadioComponentController::_writeCalibration(void)
{
	...
	else
	{
		...
		for (int chan = 0; chan < _chanMax(); chan++)
		{
			...
			if (_px4Vehicle() && _apmPossibleMissingRCChannelParams.contains(chan + 1)
					&& !parameterExists(FactSystem::defaultComponentId, minTpl.arg(chan + 1)))
			{
