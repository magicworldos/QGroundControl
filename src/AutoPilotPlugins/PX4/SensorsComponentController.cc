...
void SensorsComponentController::_handleUASTextMessage(int uasId, int compId, int severity, QString text)
{
		...
		if (text == "accel" || text == "mag" || text == "gyro")
		{
			...
			else if (text == "mag")
			{
				...
				if (_vehicle->parameterManager()->parameterExists(FactSystem::defaultComponentId, "CAL_MAG_SIDES"))
				{
					...
					sides = _vehicle->parameterManager()->getParameter(FactSystem::defaultComponentId,
							"CAL_MAG_SIDES")->rawValue().toFloat();
				}
	...
	{
		_orientationCalAreaHelpText->setProperty("text",
				tr("Orientation already completed, place you vehicle into one of the incomplete orientations shown below and hold it still"));
		return;
	}
...
void SensorsComponentController::_refreshParams(void)
{
	...
	fastRefreshList << "CAL_MAG0_ID" << "CAL_MAG1_ID" << "CAL_MAG2_ID" << "CAL_MAG0_ROT" << "CAL_MAG1_ROT" <<
			"CAL_MAG2_ROT";
