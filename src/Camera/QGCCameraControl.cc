...
//-----------------------------------------------------------------------------
QGCCameraControl::QGCCameraControl(const mavlink_camera_information_t *info, Vehicle *vehicle, int compID,
				   QObject *parent)
	: FactGroup(0, parent)
	, _vehicle(vehicle)
	, _compID(compID)
...
QGCCameraControl::takePhoto()
{
	...
	if (!capturesPhotos() || (cameraMode() == CAM_MODE_VIDEO && !photosInVideoMode())
			|| photoStatus() != PHOTO_CAPTURE_IDLE)
	{
		return false;
	}
	...
	if (capturesPhotos())
	{
		_vehicle->sendMavCommand(
			...
			false,                                                      // ShowError
			0,                                                          // Reserved (Set to 0)
			_photoMode == PHOTO_CAPTURE_SINGLE ? 0 :
			_photoLapse,       // Duration between two consecutive pictures (in seconds--ignored if single image)
			_photoMode == PHOTO_CAPTURE_SINGLE ? 1 :
...
QGCCameraControl::stopTakePhoto()
{
	...
	if (photoStatus() == PHOTO_CAPTURE_IDLE || (photoStatus() != PHOTO_CAPTURE_INTERVAL_IDLE
			&& photoStatus() != PHOTO_CAPTURE_INTERVAL_IN_PROGRESS))
	{
		return false;
	}
...
QGCCameraControl::_updateRanges(Fact *pFact)
{
	...
	foreach (QGCCameraOptionRange *pRange, _optionRanges)
	{
		...
		if (!changedList.contains(pRange->targetParam) && (pRange->param == pFact->name()
				|| pRange->condition.contains(pFact->name())))
		{
...
{
	qCDebug(CameraControlLog) << "_handleStorageInfo:" << st.available_capacity << st.status << st.storage_count <<
...
QGCCameraControl::handleCaptureStatus(const mavlink_camera_capture_status_t &cap)
{
	...
	qCDebug(CameraControlLog) << "handleCaptureStatus:" << cap.available_capacity << cap.image_interval << cap.image_status
				  << cap.recording_time_ms << cap.video_status;
	...
	uint8_t vs = cap.video_status < (uint8_t)VIDEO_CAPTURE_STATUS_LAST ? cap.video_status :
		     (uint8_t)VIDEO_CAPTURE_STATUS_UNDEFINED;
	uint8_t ps = cap.image_status < (uint8_t)PHOTO_CAPTURE_LAST ? cap.image_status : (uint8_t)
...
QGCCameraControl::_loadRanges(QDomNode option, const QString factName, QString paramValue)
{
	...
	if (rangeRoot.size())
	{
		...
		for (int i = 0; i < parameterRanges.size(); i++)
		{
			...
			if (optNames.size())
			{
				QGCCameraOptionRange *pRange = new QGCCameraOptionRange(this, factName, paramValue, param, condition, optNames,
						optValues);
				_optionRanges.append(pRange);
				qCDebug(CameraControlLogVerbose) << "New range limit:" << factName << paramValue << param << condition << optNames <<
								 optValues;
			}
		}
	}
...
//-----------------------------------------------------------------------------
bool
QGCCameraControl::_loadNameValue(QDomNode option, const QString factName, FactMetaData *metaData, QString &optName,
...
QGCCameraControl::_downloadFinished()
{
	...
	{
		data.clear();
		qWarning() << QString("Camera Definition download error: %1 status: %2").arg(reply->errorString(),
