...
VERSION_REXP("^(APM:Copter|APM:Plane|APM:Rover|APM:Sub|ArduCopter|ArduPlane|ArduRover|ArduSub) +[vV](\\d*)\\.*(\\d*)*\\.*(\\d*)*");
...
const int   APMFirmwarePlugin::_artooVideoHandshakePort =
	5502;       ///< Port for video handshake on ARTOO controller
...
{
	qmlRegisterType<APMFlightModesComponentController> ("QGroundControl.Controllers", 1, 0,
...
void APMFirmwarePlugin::_handleOutgoingParamSet(Vehicle *vehicle, LinkInterface *outgoingLink,
...
{
	QString messageText;
	APMFirmwarePluginInstanceData *instanceData = qobject_cast<APMFirmwarePluginInstanceData *>
	...
	if (vehicle->firmwareMajorVersion() == Vehicle::versionNotSetValue || statusText.severity < MAV_SEVERITY_NOTICE)
	{
		...
		if (!messageText.contains(APM_SOLO_REXP))
		{
			...
			if (messageText.contains(APM_COPTER_REXP) || messageText.contains(APM_PLANE_REXP)
					|| messageText.contains(APM_ROVER_REXP) || messageText.contains(APM_SUB_REXP))
			{
				...
				vehicle->setFirmwareVersion(firmwareVersion.majorNumber(), firmwareVersion.minorNumber(),
							    firmwareVersion.patchNumber());
				...
				if (supportedMajorNumber != -1)
				{
					...
					{
						qgcApp()->showMessage(
							tr("QGroundControl fully supports Version %1.%2 and above. You are using a version prior to that. This combination is untested, you may run into unpredictable results.").arg(
								supportedMajorNumber).arg(supportedMinorNumber));
					}
				}
			}
		}
	...
	if (messageText.contains(APM_COPTER_REXP) || messageText.contains(APM_PLANE_REXP)
			|| messageText.contains(APM_ROVER_REXP) || messageText.contains(APM_SUB_REXP) ||
			messageText.contains(APM_PX4NUTTX_REXP) || messageText.contains(APM_FRAME_REXP)
			|| messageText.contains(APM_SYSID_REXP))
	{
	...
	else if (messageText.contains(APM_FRAME_REXP))
	{
		...
		if (frameTypeRegex.indexIn(messageText) != -1)
		{
			...
			if (!frameType.isEmpty() && (frameType == QStringLiteral("Y6") || frameType == QStringLiteral("OCTA_QUAD")
	...
	if (messageText.startsWith("PreArm"))
	{
		...
		if (instanceData->noisyPrearmMap.contains(messageText)
				&& instanceData->noisyPrearmMap[messageText].msecsTo(QTime::currentTime()) < (10 * 1000))
		{
			return false;
		}
...
void APMFirmwarePlugin::adjustOutgoingMavlinkMessage(Vehicle *vehicle, LinkInterface *outgoingLink,
		mavlink_message_t *message)
{
	//-- Don't process messages to/from UDP Bridge. It doesn't suffer from these issues
	if (message->compid == MAV_COMP_ID_UDP_BRIDGE)
...
void APMFirmwarePlugin::_soloVideoHandshake(Vehicle *vehicle)
{
	...
	QObject::connect(socket, static_cast<void (QTcpSocket::*)(QAbstractSocket::SocketError)>(&QTcpSocket::error), this,
			 &APMFirmwarePlugin::_artooSocketError);
}
...
QString APMFirmwarePlugin::internalParameterMetaDataFile(Vehicle *vehicle)
{
	...
	case MAV_TYPE_SUBMARINE:
		if (vehicle->firmwareMajorVersion() < 3 || (vehicle->firmwareMajorVersion() == 3
				&& vehicle->firmwareMinorVersion() <= 4))
		{
...
double APMFirmwarePlugin::minimumTakeoffAltitude(Vehicle *vehicle)
{
	...
	{
		minTakeoffAlt = vehicle->parameterManager()->getParameter(FactSystem::defaultComponentId,
				takeoffAltParam)->rawValue().toDouble() / paramDivisor;
	}
