...
void APMParameterMetaData::addMetaDataToFact(Fact *fact, MAV_TYPE vehicleType)
{
	...
	if (!rawMetaData->incrementSize.isEmpty())
	{
		...
		{
			qCDebug(APMParameterMetaDataLog) << "Invalid value for increment, name:" << metaData->name() << " increment:" <<
							 rawMetaData->incrementSize;
		}
	}
...
void APMParameterMetaData::getParameterMetaDataVersionInfo(const QString &metaDataFile, int &majorVersion,
		int &minorVersion)
{
	majorVersion = -1;
	minorVersion = -1;
