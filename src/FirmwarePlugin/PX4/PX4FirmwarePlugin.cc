...
{
	qmlRegisterType<PX4AdvancedFlightModesController> ("QGroundControl.Controllers", 1, 0,
...
FactMetaData *PX4FirmwarePlugin::getMetaDataForFact(QObject *parameterMetaData, const QString &name,
...
void PX4FirmwarePlugin::getParameterMetaDataVersionInfo(const QString &metaDataFile, int &majorVersion,
		int &minorVersion)
{
...
QList<MAV_CMD> PX4FirmwarePlugin::supportedMissionCommands(void)
{
	...
	list << MAV_CMD_NAV_WAYPOINT
	     ...
	     << MAV_CMD_IMAGE_START_CAPTURE << MAV_CMD_IMAGE_STOP_CAPTURE << MAV_CMD_VIDEO_START_CAPTURE <<
...
void PX4FirmwarePlugin::guidedModeOrbit(Vehicle *vehicle, const QGeoCoordinate &centerCoord, double radius,
...
void PX4FirmwarePlugin::_mavCommandResult(int vehicleId, int component, int command, int result,
		bool noReponseFromVehicle)
{
	Q_UNUSED(vehicleId);
	Q_UNUSED(component);
...
double PX4FirmwarePlugin::minimumTakeoffAltitude(Vehicle *vehicle)
{
	...
	{
		return vehicle->parameterManager()->getParameter(FactSystem::defaultComponentId,
				takeoffAltParam)->rawValue().toDouble();
	}
...
void PX4FirmwarePlugin::_handleAutopilotVersion(Vehicle *vehicle, mavlink_message_t *message)
{
	...
	PX4FirmwarePluginInstanceData *instanceData = qobject_cast<PX4FirmwarePluginInstanceData *>
	...
	if (!instanceData->versionNotified)
	{
		...
		if (notifyUser)
		{
			...
			qgcApp()->showMessage(
				tr("QGroundControl supports PX4 Pro firmware Version %1.%2.%3 and above. You are using a version prior to that which will lead to unpredictable results. Please upgrade your firmware.").arg(
					supportedMajorVersion).arg(supportedMinorVersion).arg(supportedPatchVersion));
		}
	}
}
...
bool PX4FirmwarePlugin::vehicleYawsToNextWaypointInMission(const Vehicle *vehicle) const
{
	...
	else
	{
		if (vehicle->multiRotor()
				&& vehicle->parameterManager()->parameterExists(FactSystem::defaultComponentId, QStringLiteral("MIS_YAWMODE")))
		{
			Fact *yawMode = vehicle->parameterManager()->getParameter(FactSystem::defaultComponentId,
					QStringLiteral("MIS_YAWMODE"));
...
QGCCameraControl *PX4FirmwarePlugin::createCameraControl(const mavlink_camera_information_t *info, Vehicle *vehicle,
		int compID, QObject *parent)
{
