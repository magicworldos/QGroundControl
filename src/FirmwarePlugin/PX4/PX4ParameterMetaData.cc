...
QVariant PX4ParameterMetaData::_stringToTypedVariant(const QString &string, FactMetaData::ValueType_t type,
		bool *convertOk)
{
	QVariant var(string);
			...
			else if (elementName == "parameter")
			{
				...
				else
				{
					...
					if (xml.attributes().hasAttribute("default") && !strDefault.isEmpty())
					{
						...
						{
							qCWarning(PX4ParameterMetaDataLog) << "Invalid default value, name:" << name << " type:" << type << " default:" <<
											   strDefault << " error:" << errorString;
						}
					}
				}
				...
				if (!badMetaData)
				{
					if (metaData)
					{
						...
						else if (elementName == "min")
						{
							...
							{
								qCWarning(PX4ParameterMetaDataLog) << "Invalid min value, name:" << metaData->name() << " type:" << metaData->type() <<
						...
						else if (elementName == "max")
						{
							...
							{
								qCWarning(PX4ParameterMetaDataLog) << "Invalid max value, name:" << metaData->name() << " type:" << metaData->type() <<
						...
						else if (elementName == "decimal")
						{
							...
							{
								qCWarning(PX4ParameterMetaDataLog) << "Invalid decimals value, name:" << metaData->name() << " type:" <<
												   metaData->type() << " decimals:" << text << " error: invalid number";
							}
		...
		else if (xml.isEndElement())
		{
			...
			if (elementName == "parameter")
			{
				...
				if (metaData->defaultValueAvailable())
				{
					...
					{
						qCWarning(PX4ParameterMetaDataLog) << "Invalid default value, name:" << metaData->name() << " type:" << metaData->type()
...
void PX4ParameterMetaData::getParameterMetaDataVersionInfo(const QString &metaDataFile, int &majorVersion,
		int &minorVersion)
{
	QFile xmlFile(metaDataFile);
