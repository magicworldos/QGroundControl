...
void FollowMe::followMeHandleManager(const QString &)
{
	...
	for (int i = 0; i < vehicles.count(); i++)
	{
		...
		if (vehicle->px4Firmware()
				&& vehicle->flightMode().compare(FirmwarePlugin::px4FollowMeFlightMode, Qt::CaseInsensitive) == 0)
		{
			_enable();
			return;
		}
...
void FollowMe::_setGPSLocation(QGeoPositionInfo geoPositionInfo)
{
	if (geoPositionInfo.isValid())
	{
		...
		{
			_motionReport.pos_std_dev[0] = _motionReport.pos_std_dev[1] = geoPositionInfo.attribute(
