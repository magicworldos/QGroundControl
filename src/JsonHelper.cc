				  ...
				  const QList<QJsonValue::Type> &types, QString &errorString)
{
	for (int i = 0; i < types.count(); i++)
	{
		...
		if (jsonObject.contains(valueKey))
		{
			...
			{
				errorString  = QObject::tr("Incorrect value type - key:type:expected %1:%2:%3").arg(valueKey).arg(
						       _jsonValueTypeToString(jsonValue.type())).arg(_jsonValueTypeToString(types[i]));
				return false;
			}
		}
	}
...
bool JsonHelper::parseEnum(const QJsonObject &jsonObject, QStringList &enumStrings, QStringList &enumValues,
			   QString &errorString, QString valueName)
{
	...
	{
		errorString = QObject::tr("enum strings/values count mismatch in %3 strings:values %1:%2").arg(enumStrings.count()).arg(
				      enumValues.count()).arg(valueName);
		return false;
	}
				     ...
				     QString           &errorString)
{
	...
	{
		errorString = QObject::tr("File version %1 is newer than current supported version %2").arg(version).arg(
				      maxSupportedVersion);
		return false;
	}
