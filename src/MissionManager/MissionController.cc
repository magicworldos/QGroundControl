...
void MissionController::_resetMissionFlightStatus(void)
{
	...
	_missionFlightStatus.vehicleSpeed =         _controllerVehicle->multiRotor()
	...
	if (_missionFlightStatus.mAhBattery != 0)
	{
		double batteryPercentRemainingAnnounce =
			qgcApp()->toolbox()->settingsManager()->appSettings()->batteryPercentRemainingAnnounce()->rawValue().toDouble();
		_missionFlightStatus.ampMinutesAvailable = (double)_missionFlightStatus.mAhBattery / 1000.0 * 60.0 * ((
...
int MissionController::insertSimpleMissionItem(QGeoCoordinate coordinate, int i)
{
	...
	if (_visualItems->count() == 1)
	{
		MavlinkQmlSingleton::Qml_MAV_CMD takeoffCmd = _controllerVehicle->vtol() ?
				MavlinkQmlSingleton::MAV_CMD_NAV_VTOL_TAKEOFF : MavlinkQmlSingleton::MAV_CMD_NAV_TAKEOFF;
...
int MissionController::insertROIMissionItem(QGeoCoordinate coordinate, int i)
{
	...
	SimpleMissionItem *newItem = new SimpleMissionItem(_controllerVehicle, this);
	newItem->setSequenceNumber(sequenceNumber);
	newItem->setCommand((MavlinkQmlSingleton::Qml_MAV_CMD)(
				    _controllerVehicle->firmwarePlugin()->supportedMissionCommands().contains((MAV_CMD)MAV_CMD_DO_SET_ROI_LOCATION) ?
...
int MissionController::insertComplexMissionItem(QString itemName, QGeoCoordinate mapCenterCoordinate, int i)
{
	...
	if (surveyStyleItem)
	{
		...
		if (_controllerVehicle->firmwarePlugin()->hasGimbal(_controllerVehicle, rollSupported, pitchSupported, yawSupported)
				&& pitchSupported)
		{
...
void MissionController::removeMissionItem(int index)
{
	...
	{
		qWarning() << "MissionController::removeMissionItem called with bad index - count:index" << _visualItems->count() <<
			   index;
		return;
	}

	bool removeSurveyStyle = _visualItems->value<SurveyMissionItem *>(index)
		...
		QString &errorString)
{
	...
	do
	{
		qCDebug(MissionControllerLog) <<
					      "Json load: simple item loop nextSimpleItemIndex:nextComplexItemIndex:nextSequenceNumber" << nextSimpleItemIndex <<
					      nextComplexItemIndex << nextSequenceNumber;
		...
		if (nextSimpleItemIndex < itemArray.count())
		{
			...
			if (item->load(itemObject, itemObject["id"].toInt(), errorString))
			{
				qCDebug(MissionControllerLog) << "Json load: adding simple item expectedSequence:actualSequence" << nextSequenceNumber
							      << item->sequenceNumber();
		...
		QString &errorString)
{
	...
	if (_masterController->offline())
	{
		...
		appSettings->offlineEditingFirmwareType()->setRawValue(AppSettings::offlineEditingFirmwareTypeFromFirmwareType((
					MAV_AUTOPILOT)json[_jsonFirmwareTypeKey].toInt()));

		if (json.contains(_jsonVehicleTypeKey))
		{
			appSettings->offlineEditingVehicleType()->setRawValue(AppSettings::offlineEditingVehicleTypeFromVehicleType((
						MAV_TYPE)json[_jsonVehicleTypeKey].toInt()));
		}
	}
	...
	for (int i = 0; i < rgMissionItems.count(); i++)
	{
		...
		if (itemType == VisualMissionItem::jsonTypeSimpleItemValue)
		{
			...
			{
				qCDebug(MissionControllerLog) << "Loading simple item: nextSequenceNumber:command" << nextSequenceNumber <<
							      simpleItem->command();
...
void MissionController::_calcPrevWaypointValues(double homeAlt, VisualMissionItem *currentItem,
		VisualMissionItem *prevItem, double *azimuth, double *distance, double *altDifference)
{
...
void MissionController::_addWaypointLineSegment(CoordVectHashTable &prevItemPairHashTable, VisualItemPair &pair)
{
	...
	else
	{
		...
		auto linevect       = new CoordinateVector(pair.first->isSimpleItem() ? pair.first->coordinate() :
				pair.first->exitCoordinate(), pair.second->coordinate(), this);
		auto originNotifier = pair.first->isSimpleItem() ? &VisualMissionItem::coordinateChanged :
...
void MissionController::_updateBatteryInfo(int waypointIndex)
{
	if (_missionFlightStatus.mAhBattery != 0)
	{
		...
		_missionFlightStatus.batteriesRequired = ceil((_missionFlightStatus.hoverAmpsTotal +
				_missionFlightStatus.cruiseAmpsTotal) / _missionFlightStatus.ampMinutesAvailable);
		...
		if (item->specifiesCoordinate())
		{
			...
			if (!item->isStandaloneCoordinate())
			{
				...
				if (lastCoordinateItem != _settingsItem || linkStartToHome)
				{
					...
					_missionFlightStatus.maxTelemetryDistance = qMax(_missionFlightStatus.maxTelemetryDistance, _calcDistanceToHome(item,
							_settingsItem));
...
void MissionController::_initVisualItem(VisualMissionItem *visualItem)
{
	...
	connect(visualItem, &VisualMissionItem::coordinateHasRelativeAltitudeChanged,       this,
		&MissionController::_recalcWaypointLines);
	connect(visualItem, &VisualMissionItem::exitCoordinateHasRelativeAltitudeChanged,   this,
		&MissionController::_recalcWaypointLines);
	...
	else
	{
		...
		if (complexItem)
		{
			...
			connect(complexItem, &ComplexMissionItem::greatestDistanceToChanged,    this,
				&MissionController::_recalcMissionFlightStatus);
			connect(complexItem, &ComplexMissionItem::additionalTimeDelayChanged,   this,
				&MissionController::_recalcMissionFlightStatus);
		}
...
void MissionController::_addMissionSettings(QmlObjectListModel *visualItems, bool addToCenter)
{
	...
	if (addToCenter)
	{
		if (visualItems->count() > 1)
		{
			...
			if (firstCoordSet)
			{
				settingsItem->setCoordinate(QGeoCoordinate((south + ((north - south) / 2)) - 90.0, (west + ((east - west) / 2)) - 180.0,
							    0.0));
			}
		}
	}
...
int MissionController::resumeMissionIndex(void) const
{
	...
	if (!_editMode)
	{
		resumeIndex = _missionManager->lastCurrentIndex() + (_controllerVehicle->firmwarePlugin()->sendHomePositionToVehicle() ?
				0 : 1);
...
void MissionController::_scanForAdditionalSettings(QmlObjectListModel *visualItems, Vehicle *vehicle)
{
	...
	while (scanIndex < visualItems->count())
	{
		...
		qCDebug(MissionControllerLog) << "MissionController::_scanForAdditionalSettings count:scanIndex" << visualItems->count()
					      << scanIndex;
