...
void MissionControllerTest::_initForFirmwareType(MAV_AUTOPILOT firmwareType)
{
	...
	QCOMPARE(_multiSpyMissionController->init(_missionController, _rgMissionControllerSignals, _cMissionControllerSignals),
		 true);

	_masterController->start(true /* editMode */);

	// All signals should some through on start
	QCOMPARE(_multiSpyMissionController->checkOnlySignalsByMask(visualItemsChangedSignalMask |
			waypointLinesChangedSignalMask), true);
...
void MissionControllerTest::_testOfflineToOnlineWorker(MAV_AUTOPILOT firmwareType)
{
	...
	_missionController->insertSimpleMissionItem(QGeoCoordinate(37.803784, -122.462276),
