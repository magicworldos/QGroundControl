...
void PlanManager::writeMissionItems(const QList<MissionItem *> &missionItems)
{
	...
	if (inProgress())
	{
		qCDebug(PlanManagerLog) << QStringLiteral("writeMissionItems %1 called while transaction in progress").arg(
						_planTypeString());
		return;
	}
...
{
	qCDebug(PlanManagerLog) << QStringLiteral("_writeMissionCount %1 count:_retryCount").arg(
					_planTypeString()) << _writeMissionItems.count() << _retryCount;
...
void PlanManager::loadFromVehicle(void)
{
	...
	if (inProgress())
	{
		qCDebug(PlanManagerLog) << QStringLiteral("loadFromVehicle %1 called while transaction in progress").arg(
						_planTypeString());
		return;
	}
...
{
	qCDebug(PlanManagerLog) << QStringLiteral("_requestList %1 _planType:_retryCount").arg(
					_planTypeString()) << _planType << _retryCount;
...
void PlanManager::_ackTimeout(void)
{
	...
	case AckMissionRequest:
		...
		else if (_itemIndicesToWrite[0] == 0)
		{
			...
			{
				_retryCount++;
				qCDebug(PlanManagerLog) << QStringLiteral("Retrying %1 MISSION_COUNT retry Count").arg(
								_planTypeString()) << _retryCount;
				_writeMissionCount();
			}
		}
		...
		else
		{
			...
			_sendError(AckTimeoutError, tr("Vehicle did not request all items from ground station: %1").arg(_ackTypeToString(
						_expectedAck)));
			_expectedAck = AckNone;
			_finishTransaction(false);
		}
	...
	// MISSION_REQUEST is expected, or MISSION_ACK to end sequence
	default:
		_sendError(AckTimeoutError, tr("Vehicle did not respond to mission item communication: %1").arg(_ackTypeToString(
					_expectedAck)));
		_expectedAck = AckNone;
		_finishTransaction(false);
	}
...
bool PlanManager::_checkForExpectedAck(AckType_t receivedAck)
{
	...
	else
	{
		...
		else
		{
			...
			qCDebug(PlanManagerLog) << QString("Out of sequence ack %1 expected:received %2:%3").arg(_planTypeString().arg(
							_ackTypeToString(_expectedAck)).arg(_ackTypeToString(receivedAck)));
		}
...
void PlanManager::_handleMissionCount(const mavlink_message_t &message)
{
	...
	if (missionCount.mission_type != _planType)
	{
		...
		qCDebug(PlanManagerLog) << QStringLiteral("_handleMissionCount %1 Incorrect mission_type received expected:actual").arg(
						_planTypeString()) << _planType << missionCount.mission_type;
		return;
	}
...
void PlanManager::_requestNextMissionItem(void)
{
	...
	qCDebug(PlanManagerLog) << QStringLiteral("_requestNextMissionItem %1 sequenceNumber:retry").arg(
					_planTypeString()) << _itemIndicesToRead[0] << _retryCount;
...
void PlanManager::_handleMissionItem(const mavlink_message_t &message, bool missionItemInt)
{
	...
	if (!_checkForExpectedAck(AckMissionItem))
	{
		...
		{
			qCDebug(PlanManagerLog) << QStringLiteral("_handleMissionItem %1 dropping spurious item seq:command:current").arg(
							_planTypeString()) << seq << command << isCurrentItem;
			return;
		}
	}

	qCDebug(PlanManagerLog) << QStringLiteral("_handleMissionItem %1 seq:command:current:ardupilotHomePositionUpdate").arg(
	...
	else
	{
		qCDebug(PlanManagerLog) <<
					QStringLiteral("_handleMissionItem %1 mission item received item index which was not requested, disregrarding:").arg(
						_planTypeString()) << seq;
...
void PlanManager::_handleMissionRequest(const mavlink_message_t &message, bool missionItemInt)
{
	...
	if (missionRequest.mission_type != _planType)
	{
		...
		// a stale message here, for example when the MAV ran into a timeout and sent a message twice
		qCDebug(PlanManagerLog) <<
					QStringLiteral("_handleMissionRequest %1 Incorrect mission_type received expected:actual").arg(
						_planTypeString()) << _planType << missionRequest.mission_type;
		return;
	}
	...
	qCDebug(PlanManagerLog) << QStringLiteral("_handleMissionRequest %1 sequenceNumber").arg(
					_planTypeString()) << missionRequest.seq;

	if (missionRequest.seq > _writeMissionItems.count() - 1)
	{
		_sendError(RequestRangeError,
			   tr("Vehicle requested item outside range, count:request %1:%2. Send to Vehicle failed.").arg(
				   _writeMissionItems.count()).arg(missionRequest.seq));
		_finishTransaction(false);
		return;
	}
	...
	{
		qCDebug(PlanManagerLog) <<
					QStringLiteral("_handleMissionRequest %1 sequence number requested which has already been sent, sending again:").arg(
						_planTypeString()) << missionRequest.seq;
	}
	...
	qCDebug(PlanManagerLog) << QStringLiteral("_handleMissionRequest %1 sequenceNumber:command").arg(
					_planTypeString()) << missionRequest.seq << item->command();
...
void PlanManager::_handleMissionAck(const mavlink_message_t &message)
{
	...
	if (missionAck.mission_type != _planType)
	{
		// if there was a previous transaction with a different mission_type, it can happen that we receive
		// a stale message here, for example when the MAV ran into a timeout and sent a message twice
		qCDebug(PlanManagerLog) << QStringLiteral("_handleMissionAck %1 Incorrect mission_type received expected:actual").arg(
						_planTypeString()) << _planType << missionAck.mission_type;
		return;
	}
	...
	case AckNone:
		...
		qCDebug(PlanManagerLog) << QStringLiteral("Vehicle sent unexpected MISSION_ACK message, error: %1").arg(
						_missionResultToString((MAV_MISSION_RESULT)missionAck.type));
		break;

	case AckMissionCount:
		// MISSION_COUNT message expected
		_sendError(VehicleError, tr("Vehicle returned error: %1.").arg(_missionResultToString((
					MAV_MISSION_RESULT)missionAck.type)));
		_finishTransaction(false);
		break;
	...
	case AckMissionRequest:
		...
		if (missionAck.type == MAV_MISSION_ACCEPTED)
		{
			...
			{
				_sendError(MissingRequestsError, tr("Vehicle did not request all items during write sequence, missed count %1.").arg(
						   _itemIndicesToWrite.count()));
				_finishTransaction(false);
			}
		}
	...
	case AckMissionClearAll:
		...
		if (missionAck.type != MAV_MISSION_ACCEPTED)
		{
			_sendError(VehicleError, tr("Vehicle returned error: %1. Vehicle remove all failed.").arg(_missionResultToString((
						MAV_MISSION_RESULT)missionAck.type)));
		}
	...
	case AckGuidedItem:
		...
		{
			_sendError(VehicleError, tr("Vehicle returned error: %1. %2Vehicle did not accept guided item.").arg(
					   _missionResultToString((MAV_MISSION_RESULT)missionAck.type)));
