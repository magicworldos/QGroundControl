...
void RallyPoint::_factSetup(void)
{
	...
	{
		_metaDataMap = FactMetaData::createMapFromJsonFile(QStringLiteral(":/json/RallyPoint.FactMetaData.json"),
				NULL /* metaDataParent */);
	}
...
{
	return QGeoCoordinate(_latitudeFact.rawValue().toDouble(), _longitudeFact.rawValue().toDouble(),
