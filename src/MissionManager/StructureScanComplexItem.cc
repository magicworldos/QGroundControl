	...
	, _gimbalYawFact(0, _gimbalYawFactName,                     FactMetaData::valueTypeDouble)
{
	...
	{
		_metaDataMap = FactMetaData::createMapFromJsonFile(QStringLiteral(":/json/StructureScan.SettingsGroup.json"),
				NULL /* QObject parent */);
	}
	...
	connect(&_layersFact,                           &Fact::valueChanged,    this,
		&StructureScanComplexItem::_recalcLayerInfo);
	connect(&_structureHeightFact,                  &Fact::valueChanged,    this,
		&StructureScanComplexItem::_recalcLayerInfo);
	...
	connect(this, &StructureScanComplexItem::altitudeRelativeChanged,       this,
		&StructureScanComplexItem::coordinateHasRelativeAltitudeChanged);
	connect(this, &StructureScanComplexItem::altitudeRelativeChanged,       this,
		&StructureScanComplexItem::exitCoordinateHasRelativeAltitudeChanged);
	...
	connect(_cameraCalc.distanceToSurface(),    &Fact::valueChanged,                this,
		&StructureScanComplexItem::_rebuildFlightPolygon);
	connect(&_cameraCalc,                       &CameraCalc::cameraNameChanged,     this,
		&StructureScanComplexItem::_resetGimbal);

	connect(&_flightPolygon,                        &QGCMapPolygon::pathChanged,    this,
		&StructureScanComplexItem::_recalcCameraShots);
	connect(_cameraCalc.adjustedFootprintSide(),    &Fact::valueChanged,            this,
		&StructureScanComplexItem::_recalcCameraShots);
	connect(&_layersFact,                           &Fact::valueChanged,            this,
		&StructureScanComplexItem::_recalcCameraShots);
...
void StructureScanComplexItem::appendMissionItems(QList<MissionItem *> &items, QObject *missionItemParent)
{
	...
	for (int layer = 0; layer < _layersFact.rawValue().toInt(); layer++)
	{
		...
		// within the layer.
		double layerAltitude = baseAltitude + (_cameraCalc.adjustedFootprintFrontal()->rawValue().toDouble() / 2.0) +
				       (layer * _cameraCalc.adjustedFootprintFrontal()->rawValue().toDouble());
...
void StructureScanComplexItem::_recalcLayerInfo(void)
{
	if (_cameraCalc.isManualCamera())
	{
		...
		_structureHeightFact.setRawValue(_layersFact.rawValue().toInt() *
						 _cameraCalc.adjustedFootprintFrontal()->rawValue().toDouble());
	...
	else
	{
		...
		_layersFact.setRawValue(qCeil(_structureHeightFact.rawValue().toDouble() /
					      _cameraCalc.adjustedFootprintFrontal()->rawValue().toDouble()));
