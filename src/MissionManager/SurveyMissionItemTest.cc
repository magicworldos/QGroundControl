...
void SurveyMissionItemTest::init(void)
{
	...
	_offlineVehicle = new Vehicle(MAV_AUTOPILOT_PX4, MAV_TYPE_QUADROTOR, qgcApp()->toolbox()->firmwarePluginManager(),
				      this);
...
void SurveyMissionItemTest::_testDirty(void)
{
	...
	QList<Fact *> rgFacts;
	rgFacts << _surveyItem->gridAltitude() << _surveyItem->gridAngle() << _surveyItem->gridSpacing() <<
		_surveyItem->turnaroundDist() << _surveyItem->cameraTriggerDistance() <<
	...
	rgFacts << _surveyItem->groundResolution() << _surveyItem->frontalOverlap() << _surveyItem->sideOverlap() <<
		_surveyItem->cameraSensorWidth() << _surveyItem->cameraSensorHeight() <<
		_surveyItem->cameraResolutionWidth() << _surveyItem->cameraResolutionHeight() << _surveyItem->cameraFocalLength() <<
		_surveyItem->cameraOrientationLandscape() <<
