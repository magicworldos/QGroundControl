...
QStringList ParameterEditorController::searchParameters(const QString &searchText, bool searchInName,
		bool searchInDescriptions)
{
	...
	foreach (const QString &paramName, _vehicle->parameterManager()->parameterNames(_vehicle->defaultComponentId()))
	{
		...
		else
		{
			...
			else if (searchInDescriptions && (fact->shortDescription().contains(searchText, Qt::CaseInsensitive)
...
void ParameterEditorController::setRCToParam(const QString &paramName)
{
	...
	if (_uas)
	{
		QGCMapRCToParamDialog *d = new QGCMapRCToParamDialog(paramName, _uas, qgcApp()->toolbox()->multiVehicleManager(),
				MainWindow::instance());
		d->exec();
	}
