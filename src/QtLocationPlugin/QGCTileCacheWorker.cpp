...
QGCCacheWorker::_updateSetTotals(QGCCachedTileSet *set)
{
	...
	QString sq =
		QString("SELECT COUNT(size), SUM(size) FROM Tiles A INNER JOIN SetTiles B on A.tileID = B.tileID WHERE B.setID = %1").arg(
			set->id());
	...
	if (subquery.exec(sq))
	{
		if (subquery.next())
		{
			...
			qCDebug(QGCTileCacheLog) << "Set" << set->id() << "Totals:" << set->savedTileCount() << " " << set->savedTileSize() <<
						 "Expected: " << set->totalTileCount() << " " << set->totalTilesSize();
			...
			quint32 ucount = 0;
			quint64 usize  = 0;
			sq = QString("SELECT COUNT(size), SUM(size) FROM Tiles WHERE tileID IN (SELECT A.tileID FROM SetTiles A join SetTiles B on A.tileID = B.tileID WHERE B.setID = %1 GROUP by A.tileID HAVING COUNT(A.tileID) = 1)").arg(
				     set->id());
...
QGCCacheWorker::_updateTotals()
{
	...
	s = QString("SELECT COUNT(size), SUM(size) FROM Tiles WHERE tileID IN (SELECT A.tileID FROM SetTiles A join SetTiles B on A.tileID = B.tileID WHERE B.setID = %1 GROUP by A.tileID HAVING COUNT(A.tileID) = 1)").arg(
		    _getDefaultTileSet());
	qCDebug(QGCTileCacheLog) << "_updateTotals(): " << s;
...
QGCCacheWorker::_getTileDownloadList(QGCMapTask *mtask)
{
	...
	QSqlQuery query(*_db);
	QString s = QString("SELECT hash, type, x, y, z FROM TilesDownload WHERE setID = %1 AND state = 0 LIMIT %2").arg(
			    task->setID()).arg(task->count());
	...
	if (query.exec(s))
	{
		...
		{
			s = QString("UPDATE TilesDownload SET state = %1 WHERE setID = %2 and hash = \"%3\"").arg((
						int)QGCTile::StateDownloading).arg(task->setID()).arg(tiles[i]->hash());
...
QGCCacheWorker::_updateTileDownloadState(QGCMapTask *mtask)
{
	...
	else
	{
		...
		{
			s = QString("UPDATE TilesDownload SET state = %1 WHERE setID = %2 AND hash = \"%3\"").arg((int)task->state()).arg(
				    task->setID()).arg(task->hash());
		}
	}
...
QGCCacheWorker::_pruneCache(QGCMapTask *mtask)
{
	...
	s = QString("SELECT tileID, size, hash FROM Tiles WHERE tileID IN (SELECT A.tileID FROM SetTiles A join SetTiles B on A.tileID = B.tileID WHERE B.setID = %1 GROUP by A.tileID HAVING COUNT(A.tileID) = 1) ORDER BY DATE ASC LIMIT 128").arg(
		    _getDefaultTileSet());
...
QGCCacheWorker::_deleteTileSet(qulonglong id)
{
	...
	s = QString("DELETE FROM Tiles WHERE tileID IN (SELECT A.tileID FROM SetTiles A JOIN SetTiles B ON A.tileID = B.tileID WHERE B.setID = %1 GROUP BY A.tileID HAVING COUNT(A.tileID) = 1)").arg(
		    id);
	query.exec(s);
			...
			if (tileCount)
			{
				...
				if (query.exec(s))
				{
					while (query.next())
					{
						...
						QString sb =
							QString("SELECT * FROM Tiles WHERE tileID IN (SELECT A.tileID FROM SetTiles A JOIN SetTiles B ON A.tileID = B.tileID WHERE B.setID = %1 GROUP BY A.tileID HAVING COUNT(A.tileID) = 1)").arg(
								setID);
						...
						if (subQuery.exec(sb))
						{
							...
							if (tilesSaved)
							{
								...
								s = QString("SELECT COUNT(size) FROM Tiles A INNER JOIN SetTiles B on A.tileID = B.tileID WHERE B.setID = %1").arg(
									    insertSetID);
