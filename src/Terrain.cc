...
void TerrainBatchManager::addQuery(ElevationProvider *elevationProvider, const QList<QGeoCoordinate> &coordinates)
{
	...
	{
		qCDebug(ElevationProviderLog) << "addQuery: elevationProvider:coordinates.count" << elevationProvider <<
					      coordinates.count();
...
{
	qCDebug(ElevationProviderLog) << "_sendNextBatch _state:_requestQueue.count:_sentRequests.count" << _stateToString(
...
void TerrainBatchManager::_batchFailed(void)
{
	...
	foreach (const SentRequestInfo_t &sentRequestInfo, _sentRequests)
	{
		...
		{
			disconnect(sentRequestInfo.elevationProvider, &ElevationProvider::destroyed, this,
...
void TerrainBatchManager::_elevationProviderDestroyed(QObject *elevationProvider)
{
	...
	while (i < _requestQueue.count())
	{
		...
		{
			qCDebug(ElevationProviderLog) << "Removing deleted provider from _requestQueue index:elevationProvider" << i <<
						      requestInfo.elevationProvider;
	...
	for (int i = 0; i < _sentRequests.count(); i++)
	{
		...
		{
			qCDebug(ElevationProviderLog) << "Zombieing deleted provider from _sentRequests index:elevatationProvider" <<
