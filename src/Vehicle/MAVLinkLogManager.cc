...
MAVLinkLogManager::setToolbox(QGCToolbox *toolbox)
{
	...
	qmlRegisterUncreatableType<MAVLinkLogManager>("QGroundControl.MAVLinkLogManager", 1, 0, "MAVLinkLogManager",
			"Reference only");
	//-- Logging location
	_ulogExtension  = ".";
...
MAVLinkLogManager::_uploadFinished()
{
	...
	{
		qCWarning(MAVLinkLogManagerLog) << QString("Log Upload Error: %1 status: %2").arg(reply->errorString(),
...
//-----------------------------------------------------------------------------
void
MAVLinkLogManager::_mavlinkLogData(Vehicle * /*vehicle*/, uint8_t /*target_system*/, uint8_t /*target_component*/,
