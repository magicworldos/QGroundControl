	...
	, _clockFactGroup(this)
{
	...
	connect(_toolbox->uasMessageHandler(), &UASMessageHandler::textMessageCountChanged,  this,
		&Vehicle::_handleTextMessage);
	connect(_toolbox->uasMessageHandler(), &UASMessageHandler::textMessageReceived,      this,
		&Vehicle::_handletextMessageReceived);
	// Now connect the new UAS
	connect(_mav, SIGNAL(attitudeChanged(UASInterface *, double, double, double, quint64)),              this,
		SLOT(_updateAttitude(UASInterface *, double, double, double, quint64)));
	connect(_mav, SIGNAL(attitudeChanged(UASInterface *, int, double, double, double, quint64)),          this,
		SLOT(_updateAttitude(UASInterface *, int, double, double, double, quint64)));
...
Vehicle::Vehicle(MAV_AUTOPILOT              firmwareType,
	...
	, _capabilityBits(_firmwareType == MAV_AUTOPILOT_ARDUPILOTMEGA ? 0 : MAV_PROTOCOL_CAPABILITY_MISSION_FENCE |
...
void Vehicle::_commonInit(void)
{
	...
	connect(_settingsManager->appSettings()->offlineEditingFirmwareType(),  &Fact::rawValueChanged, this,
		&Vehicle::_offlineFirmwareTypeSettingChanged);
	connect(_settingsManager->appSettings()->offlineEditingVehicleType(),   &Fact::rawValueChanged, this,
...
void Vehicle::_handleGpsRawInt(mavlink_message_t &message)
{
	...
	_gpsFactGroup.hdop()->setRawValue(gpsRawInt.eph == UINT16_MAX ? std::numeric_limits<double>::quiet_NaN() :
					  gpsRawInt.eph / 100.0);
	_gpsFactGroup.vdop()->setRawValue(gpsRawInt.epv == UINT16_MAX ? std::numeric_limits<double>::quiet_NaN() :
					  gpsRawInt.epv / 100.0);
	_gpsFactGroup.courseOverGround()->setRawValue(gpsRawInt.cog == UINT16_MAX ? std::numeric_limits<double>::quiet_NaN() :
			gpsRawInt.cog / 100.0);
...
void Vehicle::_handleHighLatency2(mavlink_message_t &message)
{
	...
	_gpsFactGroup.hdop()->setRawValue(highLatency2.eph == UINT8_MAX ? std::numeric_limits<double>::quiet_NaN() :
					  highLatency2.eph / 10.0);
	_gpsFactGroup.vdop()->setRawValue(highLatency2.epv == UINT8_MAX ? std::numeric_limits<double>::quiet_NaN() :
					  highLatency2.epv / 10.0);
						...
						HL_FAILURE_FLAG_OFFBOARD_LINK = 512, /* Offboard link failure. | */
						{
							HL_FAILURE_FLAG_ENGINE = 1024, /* Engine failure. | */
							{
								...
								void Vehicle::_setCapabilities(uint64_t capabilityBits)
								{
									...
									qCDebug(VehicleLog) << QString("Vehicle %1 Mavlink 2.0").arg(_capabilityBits & MAV_PROTOCOL_CAPABILITY_MAVLINK2 ?
											    supports : doesNotSupport);
									qCDebug(VehicleLog) << QString("Vehicle %1 MISSION_ITEM_INT").arg(_capabilityBits & MAV_PROTOCOL_CAPABILITY_MISSION_INT
											    ? supports : doesNotSupport);
									qCDebug(VehicleLog) << QString("Vehicle %1 GeoFence").arg(_capabilityBits & MAV_PROTOCOL_CAPABILITY_MISSION_FENCE ?
											    supports : doesNotSupport);
									qCDebug(VehicleLog) << QString("Vehicle %1 RallyPoints").arg(_capabilityBits & MAV_PROTOCOL_CAPABILITY_MISSION_RALLY ?
											    supports : doesNotSupport);
								}
								...
								void Vehicle::_handleSysStatus(mavlink_message_t &message)
								{
									...
									else
									{
										...
										_batteryFactGroup.instantPower()->setRawValue((float)(sysStatus.current_battery * sysStatus.voltage_battery) /
												(100000.0));
									}
								...
								void Vehicle::_addNewMapTrajectoryPoint(void)
								{
									if (_mapTrajectoryHaveFirstCoordinate)
									{
										...
										_flightDistanceFact.setRawValue(_flightDistanceFact.rawValue().toDouble() + _mapTrajectoryLastCoordinate.distanceTo(
															_coordinate));
									}
									...
									if (_parameterManager->parametersReady() && _vehicleCapabilitiesKnown)
									{
										...
										if (_settingsManager->appSettings()->autoLoadMissions()->rawValue().toBool())
										{
											...
											if (!missionAutoLoadDirPath.isEmpty())
											{
												...
												QString autoloadFilename = missionAutoLoadDir.absoluteFilePath(tr("AutoLoad%1.%2").arg(_id).arg(
...
/// Returns the string to speak to identify the vehicle
								...
								void Vehicle::guidedModeGotoLocation(const QGeoCoordinate & gotoCoord)
								{
									...
									{
										qgcApp()->showMessage(QString("New location is too far. Must be less than %1 %2").arg(qRound(
													      FactMetaData::metersToAppSettingsDistanceUnits(maxDistance).toDouble())).arg(
													      FactMetaData::appSettingsDistanceUnitsString()));
										return;
									}
								...
								void Vehicle::_sendMavCommandAgain(void)
								{
									...
									if (_mavCommandRetryCount++ > _mavCommandMaxRetryCount)
									{
										if (queuedCommand.command == MAV_CMD_REQUEST_AUTOPILOT_CAPABILITIES)
										{
											...
											qCDebug(VehicleLog) <<
													    "Vehicle failed to responded to MAV_CMD_REQUEST_AUTOPILOT_CAPABILITIES. Setting no capabilities. Starting Plan request.";
											_setCapabilities(0);
											_startPlanRequest();
										}
										...
										{
											qgcApp()->showMessage(tr("Vehicle did not respond to command: %1").arg(_toolbox->missionCommandTree()->friendlyName(
									...
									if (_mavCommandRetryCount > 1)
									{
										...
										qCDebug(VehicleLog) << "Vehicle::_sendMavCommandAgain retrying command:_mavCommandRetryCount" << queuedCommand.command
								...
								void Vehicle::_handleCommandAck(mavlink_message_t &message)
								{
									...
									if (ack.command == MAV_CMD_REQUEST_AUTOPILOT_CAPABILITIES && ack.result != MAV_RESULT_ACCEPTED)
									{
										// We aren't going to get a response back for capabilities, so stop waiting for it before we ask for mission items
										qCDebug(VehicleLog) <<
												    QStringLiteral("Vehicle responded to MAV_CMD_REQUEST_AUTOPILOT_CAPABILITIES with error(%1). Setting no capabilities. Starting Plan request.").arg(
													    ack.result);
										_setCapabilities(0);
									}
									...
									if (ack.command == MAV_CMD_REQUEST_PROTOCOL_VERSION && ack.result != MAV_RESULT_ACCEPTED)
									{
										...
										// MAVLink 1
										qCDebug(VehicleLog) << QStringLiteral("Vehicle responded to MAV_CMD_REQUEST_PROTOCOL_VERSION with error(%1).").arg(
													    ack.result);
...
// Temporarily removed, waiting for new command implementation
								...
								{
									doCommandLongUnverified(_defaultComponentId, MAV_CMD_DO_MOTOR_TEST, motor, MOTOR_TEST_THROTTLE_PERCENT, percent,
												timeoutSecs);
								}
#endif
								...
								QString Vehicle::hobbsMeter()
								{
											...
											_parameterManager->parameterExists(FactSystem::defaultComponentId, HOOBS_LO))
									{
										...
										uint64_t hobbsTimeSeconds = ((uint64_t)factHi->rawValue().toUInt() << 32 | (uint64_t)factLo->rawValue().toUInt()) /
													    1000000;
