...
bool Bootloader::_ihxProgram(QSerialPort *port, const FirmwareImage *image)
{
	...
	for (uint16_t index = 0; index < image->ihxBlockCount(); index++)
	{
		...
		qCDebug(FirmwareUpgradeVerboseLog) << QString("Bootloader::_ihxProgram - address:0x%1 size:%2 block:%3").arg(
				flashAddress, 8, 16, QLatin1Char('0')).arg(bytes.count()).arg(index);
...
bool Bootloader::_ihxVerifyBytes(QSerialPort *port, const FirmwareImage *image)
{
	...
	for (uint16_t index = 0; index < image->ihxBlockCount(); index++)
	{
		...
		qCDebug(FirmwareUpgradeLog) << QString("Bootloader::_ihxVerifyBytes - address:0x%1 size:%2 block:%3").arg(readAddress,
					    8, 16, QLatin1Char('0')).arg(imageBytes.count()).arg(index);
