	...
	while (true)
	{
		...
		if (recordType == 0)
		{
			...
			else
			{
				...
				_ihxBlocks += block;
				qCDebug(FirmwareUpgradeVerboseLog) << QString("_ihxLoad - new block - address:%1 size:%2 block:%3").arg(address).arg(
						blockByteCount).arg(ihxBlockCount());
			}
...
bool FirmwareImage::_px4Load(const QString &imageFilename)
{
	...
	if (success)
	{
		...
		{
			emit statusMessage(tr("Unable to open parameter meta data file %1 for writing, error: %2").arg(parameterFilename).arg(
						   parameterFile.errorString()));
		}
	...
	if (success)
	{
		...
		{
			emit statusMessage(tr("Unable to open airframe meta data file %1 for writing, error: %2").arg(airframeFilename).arg(
						   airframeFile.errorString()));
		}
	}
	...
	{
		emit statusMessage(tr("Unable to open decompressed file %1 for writing, error: %2").arg(decompressFilename).arg(
