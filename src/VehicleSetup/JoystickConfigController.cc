...
const int JoystickConfigController::_calMoveDelta =         32768 /
		2;  ///< Amount of delta past center which is considered stick movement
const int JoystickConfigController::_calSettleDelta =
	600;        ///< Amount of delta which is considered no stick movement
...
void JoystickConfigController::nextButtonClicked(void)
{
	if (_currentStep == -1)
	{
		...
		{
			qgcApp()->showMessage(tr("Detected %1 joystick axes. To operate PX4, you need at least %2 axes.").arg(_axisCount).arg(
						      _axisMinimum));
			return;
		}
...
bool JoystickConfigController::_stickSettleComplete(int axis, int value)
{
	...
	if (abs(_stickDetectValue - value) > _calSettleDelta)
	{
		...
		qCDebug(JoystickConfigControllerLog) << "_stickSettleComplete still moving, axis:_stickDetectValue:value" << axis <<
	...
	else
	{
		...
		if (_stickDetectSettleStarted)
		{
			...
			if (_stickDetectSettleElapsed.elapsed() > _stickDetectSettleMSecs)
			{
				...
				qCDebug(JoystickConfigControllerLog) << "_stickSettleComplete detection complete, axis:_stickDetectValue:value" << axis
								     << _stickDetectValue << value;
				return true;
			}
		}
		...
		else
		{
			...
			qCDebug(JoystickConfigControllerLog) << "_stickSettleComplete starting settle timer, axis:_stickDetectValue:value" <<
							     axis << _stickDetectValue << value;
...
void JoystickConfigController::_inputStickDetect(Joystick::AxisFunction_t function, int axis, int value)
{
	...
	if (_stickDetectAxis == _axisNoAxis)
	{
		...
		if (abs(_axisValueSave[axis] - value) > _calMoveDelta)
		{
			...
			qCDebug(JoystickConfigControllerLog) << "_inputStickDetect starting settle wait, function:axis:value" << function <<
							     axis << value;
	...
	else if (axis == _stickDetectAxis)
	{
		if (_stickSettleComplete(axis, value))
		{
			...
			qCDebug(JoystickConfigControllerLog) << "_inputStickDetect saving values, function:axis:value:reversed:_axisValueSave"
							     << function << axis << value << info->reversed << _axisValueSave[axis];
...
void JoystickConfigController::_inputStickMin(Joystick::AxisFunction_t function, int axis, int value)
{
	...
	if (_stickDetectAxis == _axisNoAxis)
	{
		...
		if (_rgAxisInfo[axis].reversed)
		{
			if (value > _calCenterPoint + _calMoveDelta)
			{
				...
				_stickDetectInitialValue = value;
				_stickDetectValue = value;
				qCDebug(JoystickConfigControllerLog) << "_inputStickMin detected movement _stickDetectAxis:_stickDetectInitialValue" <<
	...
	else
	{
		...
		if (_stickSettleComplete(axis, value))
		{
			...
			qCDebug(JoystickConfigControllerLog) << "_inputStickMin saving values, function:axis:value:reversed" << function << axis
							     << value << info->reversed;
...
void JoystickConfigController::_inputCenterWait(Joystick::AxisFunction_t function, int axis, int value)
{
	...
	if (_stickDetectAxis == _axisNoAxis)
	{
		...
		int roughCenter = getDeadbandToggle() ? std::max(_rgAxisInfo[axis].deadband,
				  _calRoughCenterDelta) : _calRoughCenterDelta;
		...
		if (abs(_calCenterPoint - value) < roughCenter)
		{
			...
			qCDebug(JoystickConfigControllerLog) <<
							     "_inputStickMin detected possible center _stickDetectAxis:_stickDetectInitialValue" << _stickDetectAxis <<
...
void JoystickConfigController::_setInternalCalibrationValuesFromSettings(void)
{
	...
	for (int axis = 0; axis < _axisCount; axis++)
	{
		...
		qCDebug(JoystickConfigControllerLog) << "Read settings name:axis:min:max:trim:reversed" << joystick->name() << axis <<
						     info->axisMin << info->axisMax << info->axisTrim << info->reversed;
	}
