...
VideoReceiver::start()
{
	...
	do
	{
		...
		{
			g_object_set(G_OBJECT(dataSource), "location", qPrintable(_uri), "latency", 17, "udp-reconnect", 1, "timeout",
				     static_cast<guint64>(5000000), NULL);
		}
...
VideoReceiver::startRecording(const QString &videoFile)
{
	...
	if (videoFile.isEmpty())
	{
		...
		_videoFile = savePath + "/" + QDateTime::currentDateTime().toString("yyyy-MM-dd_hh.mm.ss") + "." +
