...
bool BluetoothLink::_hardwareConnect()
{
	...
	QObject::connect(_discoveryAgent, &QBluetoothServiceDiscoveryAgent::serviceDiscovered, this,
			 &BluetoothLink::serviceDiscovered);
...
#else
	_createSocket();
	_targetSocket->connectToService(QBluetoothAddress(_config->device().address),
...
void BluetoothLink::_createSocket()
{
	...
	QObject::connect(_targetSocket, static_cast<void (QBluetoothSocket::*)(QBluetoothSocket::SocketError)>
...
void BluetoothConfiguration::startScan()
{
	if (!_deviceDiscover)
	{
		...
		connect(_deviceDiscover, &QBluetoothDeviceDiscoveryAgent::finished,          this,
			&BluetoothConfiguration::doneScanning);
		emit scanningChanged();
	}
