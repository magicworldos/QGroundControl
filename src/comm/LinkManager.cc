...
void LinkManager::_addLink(LinkInterface *link)
{
	...
	connect(link, &LinkInterface::communicationError,   _app,
	...
	foreach (QGCSerialPortInfo portInfo, portList)
	{
		...
		if (portInfo.systemLocation().trimmed() == _autoConnectSettings->autoConnectNmeaPort()->cookedValueString())
		{
			...
			if (portInfo.getBoardInfo(boardType, boardName))
			{
				...
				else if (++_autoconnectWaitList[portInfo.systemLocation()] * _autoconnectUpdateTimerMSecs >
...
void LinkManager::_activeLinkCheck(void)
{
	...
	if (!found && link)
	{
		...
		qgcApp()->showMessage(foundNSHPrompt ?
				      ...
				      tr("Your Vehicle is not responding. If this continues, shutdown %1, restart the Vehicle letting it boot completely, then start %1.").arg(
