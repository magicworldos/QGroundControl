...
void LogReplayLink::_readNextLogEntry(void)
{
	...
	if (_logTimestamped)
	{
		...
		while (timeToNextExecutionMSecs < 3)
		{
			...
			emit playbackPercentCompleteChanged(((float)(_logCurrentTimeUSecs - _logStartTimeUSecs) /
							     (float)_logDurationUSecs) * 100);
...
void LogReplayLink::_play(void)
{
	...
	_playbackStartTimeMSecs = (quint64)QDateTime::currentMSecsSinceEpoch() - ((_logCurrentTimeUSecs - _logStartTimeUSecs) /
				  1000);
