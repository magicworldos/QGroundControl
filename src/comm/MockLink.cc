...
MockLink::MockLink(SharedLinkConfigurationPointer &config)
	...
	, _vehicleLatitude(_defaultVehicleLatitude + ((_vehicleSystemId - 128) *
			   0.0001))                           // Slight offset for each vehicle
...
void MockLink::_paramRequestListWorker(void)
{
	...
	if ((_failureMode == MockConfiguration::FailMissingParamOnInitialReqest
			|| _failureMode == MockConfiguration::FailMissingParamOnAllRequests) && paramName == _failParam)
	{
...
void MockLink::_handleParamRequestRead(const mavlink_message_t &msg)
{
	...
	const QString paramName(QString::fromLocal8Bit(request.param_id, strnlen(request.param_id,
...
void MockLink::_respondWithAutopilotVersion(void)
{
	...
	mavlink_msg_autopilot_version_pack_chan(_vehicleSystemId,
						...
						_mavlinkChannel,
						&msg,
						MAV_PROTOCOL_CAPABILITY_MAVLINK2 | (_firmwareType == MAV_AUTOPILOT_ARDUPILOTMEGA ? 0 :
...
void MockLink::_handleLogRequestData(const mavlink_message_t &msg)
{
	...
	{
		qWarning() << "MockLink::_handleLogRequestData offset past end of file request.ofs:size" << request.ofs <<
