...
const size_t MockLinkFileServer::cFailureModes = sizeof(MockLinkFileServer::rgFailureModes) / sizeof(
			MockLinkFileServer::rgFailureModes[0]);
...
void MockLinkFileServer::_listCommand(uint8_t senderSystemId, uint8_t senderComponentId, FileManager::Request *request,
				      uint16_t seqNumber)
{
...
void MockLinkFileServer::_openCommand(uint8_t senderSystemId, uint8_t senderComponentId, FileManager::Request *request,
				      uint16_t seqNumber)
{
...
void MockLinkFileServer::_readCommand(uint8_t senderSystemId, uint8_t senderComponentId, FileManager::Request *request,
				      uint16_t seqNumber)
{
	FileManager::Request	response;
	uint16_t				outgoingSeqNumber = _nextSeqNumber(seqNumber);
...
void MockLinkFileServer::_streamCommand(uint8_t senderSystemId, uint8_t senderComponentId,
					FileManager::Request *request, uint16_t seqNumber)
{
	uint16_t                outgoingSeqNumber = _nextSeqNumber(seqNumber);
	FileManager::Request    response;
...
void MockLinkFileServer::_terminateCommand(uint8_t senderSystemId, uint8_t senderComponentId,
		FileManager::Request *request, uint16_t seqNumber)
{
	uint16_t outgoingSeqNumber = _nextSeqNumber(seqNumber);
	...
	if (request->hdr.session != _sessionId)
	{
		_sendNak(senderSystemId, senderComponentId, FileManager::kErrInvalidSession, outgoingSeqNumber,
			 FileManager::kCmdTerminateSession);
		return;
	}
...
void MockLinkFileServer::handleFTPMessage(const mavlink_message_t &message)
{
	...
	if (request->hdr.opcode != FileManager::kCmdResetSessions && request->hdr.opcode != FileManager::kCmdTerminateSession)
	{
		...
		else if (_errMode == errModeNakResponse)
		{
			...
			_sendNak(message.sysid, message.compid, FileManager::kErrFail, outgoingSeqNumber,
				 (FileManager::Opcode)request->hdr.opcode);
			return;
		}
	}
	...
	default:
		...
		_sendNak(message.sysid, message.compid, FileManager::kErrUnknownCommand, outgoingSeqNumber,
			 (FileManager::Opcode)request->hdr.opcode);
		break;
	}
}
...
/// @brief Sends an Ack
void MockLinkFileServer::_sendAck(uint8_t targetSystemId, uint8_t targetComponentId, uint16_t seqNumber,
				  FileManager::Opcode reqOpcode)
{
	FileManager::Request ackResponse;
...
void MockLinkFileServer::_sendNak(uint8_t targetSystemId, uint8_t targetComponentId, FileManager::ErrorCode error,
				  uint16_t seqNumber, FileManager::Opcode reqOpcode)
{
...
void MockLinkFileServer::_sendResponse(uint8_t targetSystemId, uint8_t targetComponentId, FileManager::Request *request,
				       uint16_t seqNumber)
{
