...
QGCFlightGearLink::QGCFlightGearLink(Vehicle *vehicle, QString startupArguments, QString remoteHost, QHostAddress host,
				     quint16 port)
	: _vehicle(vehicle)
	, _udpCommSocket(NULL)
	, _fgProcess(NULL)
	...
	, barometerOffsetkPa(0.0f)
{
	...
	connect(this, &QGCFlightGearLink::showCriticalMessageFromThread, qgcApp(),
...
void QGCFlightGearLink::processError(QProcess::ProcessError err)
{
	...
	case QProcess::Crashed:
		emit showCriticalMessageFromThread(tr("FlightGear Crashed"),
						   tr("This is a FlightGear-related problem. Please upgrade FlightGear"));
		break;

	case QProcess::Timedout:
		emit showCriticalMessageFromThread(tr("FlightGear Start Timed Out"),
						   tr("Please check if the path and command is correct"));
		break;

	case QProcess::WriteError:
		emit showCriticalMessageFromThread(tr("Could not Communicate with FlightGear"),
						   tr("Please check if the path and command is correct"));
		break;
...
void QGCFlightGearLink::updateControls(quint64 time, float rollAilerons, float pitchElevator, float yawRudder,
				       float throttle, quint8 systemMode, quint8 navMode)
{
	...
	{
		qDebug() << "HIL: Got NaN values from the hardware: isnan output: roll: " << qIsNaN(
				 rollAilerons) << ", pitch: " << qIsNaN(pitchElevator) << ", yaw: " << qIsNaN(yawRudder) << ", throttle: " << qIsNaN(
				 throttle);
	}
}
...
void QGCFlightGearLink::readBytes()
{
	...
	if (values.size() != nValues)
	{
		...
		emit showCriticalMessageFromThread(tr("FlightGear HIL"),
						   tr("Flight Gear protocol file '%1' is out of date. Quit %2. Delete the file and restart %2 to fix.").arg(
								   _fgProtocolFileFullyQualified).arg(qgcApp()->applicationName()));
		disconnectSimulation();
		return;
	}
	...
	if (_sensorHilEnabled)
	{
		...
		emit sensorHilRawImuChanged(QGC::groundTimeUsecs(), xacc, yacc, zacc, rollspeed, pitchspeed, yawspeed,
					    xmag_body, ymag_body, zmag_body, abs_pressure * 1e-2f, diff_pressure * 1e-2f, pressure_alt, temperature,
					    fields_changed); //Pressure in hPa for _vehicle->uas()link
...
//        qDebug()  << "sensorHilRawImuChanged " << xacc  << yacc << zacc  << rollspeed << pitchspeed << yawspeed << xmag << ymag << zmag << abs_pressure << diff_pressure << pressure_alt << temperature;
		...
		emit sensorHilGpsChanged(QGC::groundTimeUsecs(), lat, lon, alt, gps_fix_type, eph, epv, vel, vx, vy, vz, cog,
					 satellites);
		...
		if (chr == ' ')
		{
			...
			else
			{
				...
				else
				{
					...
					if (inQuotedString)
					{
						QGCMessageBox::critical(tr("FlightGear HIL"),
									tr("FlightGear failed to start. There are mismatched quotes in specified command line options"));
						return false;
					}
...
#error Unknown OS build flavor
	...
	while (!QFileInfo(fgAppFullyQualified).exists())
	{
		...
		QString dirPath = QGCQFileDialog::getExistingDirectory(MainWindow::instance(),
				  tr("Please select directory of FlightGear application : ") + fgAppName);
	...
	{
		QGCMessageBox::critical(tr("FlightGear HIL"),
					tr("Incorrect %1 installation. Aircraft directory is missing: '%2'.").arg(qgcApp()->applicationName()).arg(
						qgcAircraftDir));
		return false;
	}
	...
	QString fgProtocol(_vehicle->vehicleType() == MAV_TYPE_QUADROTOR ? "qgroundcontrol-quadrotor" :
	...
	{
		QGCMessageBox::critical(tr("FlightGear HIL"),
					tr("Incorrect FlightGear setup. Protocol directory is missing: '%1'. Command line parameter for --fg-root may be set incorrectly.").arg(
						fgProtocolDir.path()));
		return false;
	}
	...
	{
		QGCMessageBox::critical(tr("FlightGear HIL"),
					tr("Incorrect installation. Protocol directory is missing (%1).").arg(qgcProtocolDir.path()));
		return false;
	}
	...
	{
		QGCMessageBox::critical(tr("FlightGear HIL"),
					tr("Incorrect installation. FlightGear protocol file missing: %1").arg(qgcProtocolFileFullyQualified));
		return false;
	}
	...
	if (QFileInfo(_fgProtocolFileFullyQualified).exists())
	{
				...
				!qgcFile.open(QIODevice::ReadOnly))
		{
			QGCMessageBox::warning(tr("FlightGear HIL"), tr("Unable to verify that protocol file %1 is current. "
					       ...
					       "Safest approach is to delete the file manually and allow %2 install the latest file.").arg(
						       qgcApp()->applicationName()).arg(_fgProtocolFileFullyQualified));
		}
		...
		if (fgBytes != qgcBytes)
		{
			QGCMessageBox::warning(tr("FlightGear HIL"),
					       tr("FlightGear protocol file %1 is out of date. It will be deleted, which will cause %2 to install the latest version of the file.").arg(
						       _fgProtocolFileFullyQualified).arg(qgcApp()->applicationName()));

			if (!QFile::remove(_fgProtocolFileFullyQualified))
			{
				QGCMessageBox::warning(tr("FlightGear HIL"),
						       tr("Delete of protocol file failed. You will have to manually delete the file."));
				return false;
			}
		}
	}
	...
	if (!QFileInfo(_fgProtocolFileFullyQualified).exists())
	{
		QMessageBox msgBox(QMessageBox::Critical,
				   ...
				   tr("FlightGear Failed to Start. %1 protocol (%2) not installed to FlightGear Protocol directory (%3)").arg(
					   qgcApp()->applicationName()).arg(fgProtocolXmlFile).arg(fgProtocolDir.path()),
				   QMessageBox::Cancel,
				   MainWindow::instance());
		msgBox.setWindowModality(Qt::ApplicationModal);
		...
		if (!succeeded)
		{
			...
			QMessageBox msgBox(QMessageBox::Critical,
					   ...
					   tr("Copy from (%1) to (%2) failed, possibly due to permissions issue. You will need to perform manually. Try pasting the following command into a Command Prompt which was started with Run as Administrator:\n\n").arg(
						   qgcProtocolFileFullyQualified).arg(_fgProtocolFileFullyQualified) + copyCmd,
#else
