	...
	, _serialConfig(qobject_cast<SerialConfiguration*>(config.data()))
{
	...
	qCDebug(SerialLinkLog) << "Create SerialLink " << _serialConfig->portName() << _serialConfig->baud() <<
			       _serialConfig->flowControl()
...
bool SerialLink::_hardwareConnect(QSerialPort::SerialPortError &error, QString &errorString)
{
	...
	if (!_port->isOpen())
	{
		qDebug() << "open failed" << _port->errorString() << _port->error() << getName() <<
			 qgcApp()->toolbox()->linkManager()->isAutoconnectLink(this);
		error = _port->error();
