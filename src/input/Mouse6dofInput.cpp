...
Mouse6dofInput::Mouse6dofInput(Mouse3DInput *mouseInput) :
	...
	cValue(0.0)
{
	connect(qgcApp()->toolbox()->multiVehicleManager(), &MultiVehicleManager::activeVehicleChanged, this,
		&Mouse6dofInput::_activeVehicleChanged);
...
void Mouse6dofInput::_activeVehicleChanged(Vehicle *vehicle)
{
	if (this->uas)
	{
		disconnect(this, SIGNAL(mouse6dofChanged(double, double, double, double, double, double)), uas,
			   SLOT(setManual6DOFControlCommands(double, double, double, double, double, double)));
	...
	if (vehicle)
	{
		...
		connect(this, SIGNAL(mouse6dofChanged(double, double, double, double, double, double)), uas,
			SLOT(setManual6DOFControlCommands(double, double, double, double, double, double)));
...
void Mouse6dofInput::handleX11Event(XEvent *event)
{
	...
	case ClientMessage:
		...
		case MagellanInputMotionEvent :
			...
			{
				MagellanEvent.MagellanData[i] = (abs(MagellanEvent.MagellanData[i]) < mouse3DMax) ? MagellanEvent.MagellanData[i] :
								(mouse3DMax * MagellanEvent.MagellanData[i] / abs(MagellanEvent.MagellanData[i]));
			}
