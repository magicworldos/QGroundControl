...
const int RadioConfigTest::_testCenterValue = RadioConfigTest::_testMinValue + ((RadioConfigTest::_testMaxValue -
		RadioConfigTest::_testMinValue) / 2);
		...
		bool identifyStep)
{
	...
	qCDebug(RadioConfigTestLog) << "_stickMoveAutoStep function:direction:reversed:identifyStep" << functionStr << function
				    << direction << identifyStep;
...
void RadioConfigTest::_fullCalibrationWorker(MAV_AUTOPILOT firmwareType)
{
	...
	for (int function = 0; function < RadioComponentController::rcCalFunctionMax; function++)
	{
		...
		for (int channel = 0; channel < _chanMax(); channel++)
		{
			if (_channelSettings()[channel].function == function)
			{
				...
				if (_px4Vehicle())
				{
					...
					{
						Q_ASSERT(_vehicle->parameterManager()->getParameter(FactSystem::defaultComponentId,
								switchParam)->rawValue().toInt() != channel + 1);
					}
				}
		...
		if (!found)
		{
			...
			if (paramName)
			{
				_rgFunctionChannelMap[function] = _vehicle->parameterManager()->getParameter(FactSystem::defaultComponentId,
								  paramName)->rawValue().toInt();
	...
	_controller->nextButtonClicked();
	_beginCalibration();
	_stickMoveAutoStep("Throttle",  RadioComponentController::rcCalFunctionThrottle,    moveToMax,
			   true /* identify step */);
	_stickMoveAutoStep("Throttle",  RadioComponentController::rcCalFunctionThrottle,    moveToMin,
			   false /* not identify step */);
	_stickMoveAutoStep("Yaw",       RadioComponentController::rcCalFunctionYaw,         moveToMax,
			   true /* identify step */);
	_stickMoveAutoStep("Yaw",       RadioComponentController::rcCalFunctionYaw,         moveToMin,
			   false /* not identify step */);
	_stickMoveAutoStep("Roll",      RadioComponentController::rcCalFunctionRoll,        moveToMax,
			   true /* identify step */);
	_stickMoveAutoStep("Roll",      RadioComponentController::rcCalFunctionRoll,        moveToMin,
			   false /* not identify step */);
	_stickMoveAutoStep("Pitch",     RadioComponentController::rcCalFunctionPitch,       moveToMax,
			   true /* identify step */);
	_stickMoveAutoStep("Pitch",     RadioComponentController::rcCalFunctionPitch,       moveToMin,
			   false /* not identify step */);
	_stickMoveAutoStep("Pitch",     RadioComponentController::rcCalFunctionPitch,       moveToCenter,
			   false /* not identify step */);
	_switchMinMaxStep();
...
void RadioConfigTest::_channelHomePosition(void)
{
	...
	_mockLink->emitRemoteControlChannelRawChanged(_rgFunctionChannelMap[RadioComponentController::rcCalFunctionThrottle],
			_testMinValue - 1);
}
...
void RadioConfigTest::_validateParameters(void)
{
	...
	for (int chanFunction = 0; chanFunction < RadioComponentController::rcCalFunctionMax; chanFunction++)
	{
		...
		if (paramName)
		{
			qCDebug(RadioConfigTestLog) << "Validate" << chanFunction << _vehicle->parameterManager()->getParameter(
							    FactSystem::defaultComponentId, paramName)->rawValue().toInt();
			QCOMPARE(_vehicle->parameterManager()->getParameter(FactSystem::defaultComponentId, paramName)->rawValue().toInt(),
	...
	for (int chan = 0; chan < _chanMax(); chan++)
	{
		...
		int rcMinActual = _vehicle->parameterManager()->getParameter(FactSystem::defaultComponentId,
				  minTpl.arg(oneBasedChannel))->rawValue().toInt(&convertOk);
		QCOMPARE(convertOk, true);
		int rcMaxActual = _vehicle->parameterManager()->getParameter(FactSystem::defaultComponentId,
				  maxTpl.arg(oneBasedChannel))->rawValue().toInt(&convertOk);
		QCOMPARE(convertOk, true);
		int rcTrimActual = _vehicle->parameterManager()->getParameter(FactSystem::defaultComponentId,
				   trimTpl.arg(oneBasedChannel))->rawValue().toInt(&convertOk);
		QCOMPARE(convertOk, true);
		...
		{
			float rcReversedFloat = _vehicle->parameterManager()->getParameter(FactSystem::defaultComponentId,
						revTplPX4.arg(oneBasedChannel))->rawValue().toFloat(&convertOk);
			QCOMPARE(convertOk, true);
		...
		{
			rcReversedActual = _vehicle->parameterManager()->getParameter(FactSystem::defaultComponentId,
					   revTplAPM.arg(oneBasedChannel))->rawValue().toBool();
		}
