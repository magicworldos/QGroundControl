...
{
	qCDebug(FileManagerLog) << QString("_openAckResponse: _currentOperation(%1) _readFileLength(%2)").arg(
					_currentOperation).arg(openAck->openFileLength);
...
void FileManager::_requestMissingData()
{
	...
	{
		qCDebug(FileManagerLog) << QString("_requestMissingData: offset(%1) size(%2)").arg(_missingData.head().offset).arg(
						_missingData.head().size);
...
{
	qCDebug(FileManagerLog) << QString("_closeDownloadSession: success(%1) missingBytes(%2)").arg(success).arg(
...
void FileManager::_downloadAckResponse(Request *readAck, bool readFile)
{
	...
	if (readAck->hdr.offset != _downloadOffset)
	{
		...
		{
			_closeDownloadSession(false /* failure */);
			_emitErrorMessage(tr("Download: Offset returned (%1) differs from offset requested/expected (%2)").arg(
						  readAck->hdr.offset).arg(_downloadOffset));
			return;
		}
		...
		else     // burst
		{
			...
			qCDebug(FileManagerLog) << QString("_downloadAckResponse: missing data: offset(%1) size(%2)").arg(
							missingData.offset).arg(missingData.size);
			_downloadOffset = readAck->hdr.offset;
			_readFileAccumulator.resize(_downloadOffset); // placeholder for the missing data
		}
	}

	qCDebug(FileManagerLog) << QString("_downloadAckResponse: offset(%1) size(%2) burstComplete(%3)").arg(
					readAck->hdr.offset).arg(readAck->hdr.size).arg(readAck->hdr.burstComplete);
	...
	{
		emit commandProgress(100 * ((float)(_readFileAccumulator.length() - _missingDownloadedBytes) /
...
void FileManager::_listAckResponse(Request *listAck)
{
	if (listAck->hdr.offset != _listOffset)
	{
		...
		_currentOperation = kCOIdle;
		_emitErrorMessage(tr("List: Offset returned (%1) differs from offset requested (%2)").arg(listAck->hdr.offset).arg(
					  _listOffset));
		return;
	}
...
void FileManager::_writeAckResponse(Request *writeAck)
{
	...
	{
		_closeUploadSession(false /* failure */);
		_emitErrorMessage(tr("Write: Offset returned (%1) differs from offset requested (%2)").arg(writeAck->hdr.offset).arg(
					  _writeOffset));
		return;
	}
	...
	{
		_closeUploadSession(false /* failure */);
		_emitErrorMessage(tr("Write: Size returned (%1) differs from size requested (%2)").arg(writeAck->writeFileLength).arg(
					  _writeSize));
		return;
	}
...
void FileManager::receiveMessage(mavlink_message_t message)
{
	...
	{
		qDebug() << "Received MAVLINK_MSG_ID_FILE_TRANSFER_PROTOCOL with incorrect target_system:" <<  data.target_system <<
			 "expected:" << _systemIdQGC;
		return;
	}
	...
	if (incomingSeqNumber != expectedSeqNumber)
	{
		...
		if (doAbort)
		{
			_emitErrorMessage(tr("Bad sequence number on received message: expected(%1) received(%2)").arg(expectedSeqNumber).arg(
						  incomingSeqNumber));
			return;
		}
	}
