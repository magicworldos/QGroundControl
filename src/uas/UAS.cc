...
void UAS::receiveMessage(mavlink_message_t message)
{
	...
	if (message.sysid == uasId && (!attitudeStamped || (attitudeStamped && (lastAttitude != 0))
		...
		case MAVLINK_MSG_ID_ATTITUDE:
			{
				...
				emit attitudeChanged(this, message.compid, QGC::limitAngleToPMPIf(attitude.roll),
						     QGC::limitAngleToPMPIf(attitude.pitch), QGC::limitAngleToPMPIf(attitude.yaw), time);
		...
		case MAVLINK_MSG_ID_HIL_CONTROLS:
			{
				...
				emit hilControlsChanged(hil.time_usec, hil.roll_ailerons, hil.pitch_elevator, hil.yaw_rudder, hil.throttle, hil.mode,
							hil.nav_mode);
			}
			break;
			       ...
			       mavlink_param_union_t &paramUnion)
{
	...
	emit parameterUpdate(uasId, compId, paramName, rawValue.param_count, rawValue.param_index, rawValue.param_type,
			     paramValue);
}
...
*/
void UAS::setExternalControlSetpoint(float roll, float pitch, float yaw, float thrust, quint16 buttons,
				     int joystickMode)
{
	if (!_vehicle)
	{
	...
	static quint8 countSinceLastTransmission =
		0; // Track how many calls to this function have occurred since the last MAVLink transmission
...
void UAS::setManual6DOFControlCommands(double x, double y, double z, double roll, double pitch, double yaw)
{
	...
	if (((base_mode & MAV_MODE_FLAG_DECODE_POSITION_MANUAL) && (base_mode & MAV_MODE_FLAG_DECODE_POSITION_SAFETY))
			|| (base_mode & MAV_MODE_FLAG_HIL_ENABLED))
	{
		...
		_vehicle->sendMessageOnLink(_vehicle->priorityLink(), message);
		qDebug() << __FILE__ << __LINE__ << ": SENT 6DOF CONTROL MESSAGES: x" << x << " y: " << y << " z: " << z << " roll: " <<
		       ...
		       float vx, float vy, float vz, float ind_airspeed, float true_airspeed, float xacc, float yacc, float zacc)
{
	...
	if (_vehicle->hilMode())
	{
		...
		mavlink_msg_hil_state_quaternion_pack_chan(mavlink->getSystemId(),
				...
				lat * 1e7f, lon * 1e7f, alt * 1000, vx * 100, vy * 100, vz * 100, ind_airspeed * 100, true_airspeed * 100,
				xacc * 1000 / 9.81, yacc * 1000 / 9.81, zacc * 1000 / 9.81);
		_vehicle->sendMessageOnLink(_vehicle->priorityLink(), msg);
	}
...
*/
#ifndef __mobile__
void UAS::sendHilSensors(quint64 time_us, float xacc, float yacc, float zacc, float rollspeed, float pitchspeed,
			 float yawspeed,
			 float xmag, float ymag, float zmag, float abs_pressure, float diff_pressure, float pressure_alt, float temperature,
			 quint32 fields_changed)
{
	if (!_vehicle)
	{
		return;
			     ...
			     float flow_comp_m_y, quint8 quality, float ground_distance)
{
	...
	if (_vehicle->hilMode())
	{
		...
		mavlink_msg_hil_optical_flow_pack_chan(mavlink->getSystemId(),
						       ...
						       _vehicle->priorityLink()->mavlinkChannel(),
						       &msg,
						       time_us, 0, 0 /* hack */, flow_x, flow_y, 0.0f /* hack */, 0.0f /* hack */, 0.0f /* hack */, 0 /* hack */, quality,
						       ground_distance);
...
#ifndef __mobile__
void UAS::sendHilGps(quint64 time_us, double lat, double lon, double alt, int fix_type, float eph, float epv, float vel,
		     float vn, float ve, float vd, float cog, int satellites)
{
	if (!_vehicle)
	{
		return;
	...
	if (_vehicle->hilMode())
	{
		...
		mavlink_msg_hil_gps_pack_chan(mavlink->getSystemId(),
					      ...
					      _vehicle->priorityLink()->mavlinkChannel(),
					      &msg,
					      time_us, fix_type, lat * 1e7, lon * 1e7, alt * 1e3, eph * 1e2, epv * 1e2, vel * 1e2, vn * 1e2, ve * 1e2, vd * 1e2,
					      course * 1e2, satellites);
...
void UAS::sendMapRCToParam(QString param_id, float scale, float value0, quint8 param_rc_channel_index, float valueMin,
			   float valueMax)
{
	if (!_vehicle)
	{
