...
void MAVLinkDecoder::receiveMessage(LinkInterface *link, mavlink_message_t message)
{
	...
	if (message.msgid == MAVLINK_MSG_ID_SYSTEM_TIME)
	{
		...
		sysDict[msgid].onboardToGCSUnixTimeOffsetAndDelay  = static_cast<qint64>(QGC::groundTimeMilliseconds() -
				(timebase.time_unix_usec + 500) / 1000);
	}
	...
	else
	{
		...
		if (QString(msgInfo->fields[fieldid].name) == QString("time_boot_ms")
				&& msgInfo->fields[fieldid].type == MAVLINK_TYPE_UINT32_T)
		{
			time = *((quint32 *)(m + msgInfo->fields[fieldid].wire_offset));
		}

		else if (QString(msgInfo->fields[fieldid].name).contains("usec")
				&& msgInfo->fields[fieldid].type == MAVLINK_TYPE_UINT64_T)
		{
