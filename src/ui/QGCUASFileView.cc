...
void QGCUASFileView::_downloadFile(void)
{
	...
	{
		qWarning() << QString("Download attempted while another command was in progress: _currentCommand(%1)").arg(
				   _currentCommand);
		return;
	}
...
void QGCUASFileView::_uploadFile(void)
{
	if (_currentCommand != commandNone)
	{
		qWarning() << QString("Upload attempted while another command was in progress: _currentCommand(%1)").arg(
				   _currentCommand);
		return;
	}
...
void QGCUASFileView::_listEntryReceived(const QString &entry)
{
	...
	{
		qWarning() << QString("List entry received while no list command in progress: _currentCommand(%1)").arg(
				   _currentCommand);
		return;
	}
